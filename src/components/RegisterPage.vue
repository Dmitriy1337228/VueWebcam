<template>
    ...
    <div v-if="allPhotosTaken && isCameraOpen" class="camera-download">
      <button  class="button" @click="postImage" >Send Images on Server</button>
    </div>
  </template>
  
  <script>
  const authInfo = Object.create(null); // global auth variable
  authInfo['mode'] = 'reg'; // for reg page
  let counter = 0; // how many photos are taken
  export default ({
    el: '#app',
    ...
    methods: {
      ...
      stopCameraStream() {
        ...
        authInfo['image'+counter] = img; // set img in gloabl auth obj
